...
class StrategyQAReader(BaseDatasetReader):
    def __init__(
        ...
        **kwargs,
        ...
        logger.info("saved custom decompositions, if there were any")
        ...
        with FileLock("cache.lock"):
            ...
            if os.path.exists("data/strategyqa/strategyqa_train_paragraphs.json"):
                with open(
                    ...
                    logger.info("updating train cache:")
                    self._paragraphs_cache.update(json.load(f))

            logger.info("train cache updated")
            ...
            logger.info("test cache updated")
    ...
    def get_paragraphs(self, **kwargs):
        ...
        # logger.info("para source: ", self._paragraphs_source)

        paragraphs = {
            None: lambda **kwargs: None,
            "IR-Q": self._ir_q, #self._mdr
            "ORA-P": self._ora_p,
    ...
    def _mdr(self, question, **kwargs):
        """
        Get paragraphs from DPR table
        """
        query = clean_query(question)	
	    
        f = open('/home/nnishika/mdrout/mdr_for_stqa.json', 'r')
        data = json.load(f)
        f.close()
    ...
    def _dpr(self, question, **kwargs):
        """
        Get paragraphs from DPR table
        """
        # query = clean_query(question)	
	    
        f = open('/home/nnishika/dpr_on_stqa.json', 'r')
        data = json.load(f)
        f.close()
    ...
    def _ora_p(self, decomposition, **kwargs):
        ...
        '''f = open("/home/nnishika/stqaout/stqa_q_to_posparas.json", "a")
        f.write("\n")
        f.write(json.dumps({question: results}))
        f.close()
